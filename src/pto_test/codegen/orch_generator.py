...
"""
    ...
    def _generate_memory_allocation(
        ...
        for i, spec in enumerate(all_specs):
            ...
            # Allocate memory
            lines.append(
                f"    void* dev_{spec.name} = runtime->host_api.device_malloc(size_{spec.name});\n"
            )
            lines.append(f"    if (!dev_{spec.name}) {{\n")
            lines.append(
                f'        std::cerr << "Error: Failed to allocate device memory for {spec.name}\\n";\n'
            )
            ...
            if is_output:
                ...
                lines.append(
                    f"    runtime->record_tensor_pair(host_{spec.name}, dev_{spec.name}, size_{spec.name});\n"
                )
            else:
                lines.append(
                    f"    runtime->host_api.copy_to_device(dev_{spec.name}, host_{spec.name}, size_{spec.name});\n"
                )
    ...
    def _generate_task_creation(
        ...
        lines.append("    //   runtime->add_successor(t0, t1);  // t0 → t1\n")
        lines.append("    //\n")
